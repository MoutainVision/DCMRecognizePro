...
////////////////////////////////////////////////////////////////////////////////
	bool xImageTIF::decode(xFile * hFile)
	{
		...
		m_extInfo.nNumFrames = TIFFNumberOfDirectories(m_tif);

		if (!TIFFSetDirectory(m_tif, (uint16)m_extInfo.nFrame))
		{
		...
		if (m_extInfo.nEscape == -1)
		{
			...
			m_bmpHead.biWidth = width;
			m_bmpHead.biHeight = height;
			m_extInfo.dwType = XIMAGE_FORMAT_TIF;
		...
		if (TIFFGetField(m_tif, TIFFTAG_XPOSITION, &offset))
		{
			m_extInfo.xOffset = (int32_t)offset;
		}
		
		if (TIFFGetField(m_tif, TIFFTAG_YPOSITION, &offset))
		{
			m_extInfo.yOffset = (int32_t)offset;
		}
		...
		else
		{
				...
				(photometric == PHOTOMETRIC_PALETTE))
			{
				if (bitspersample == 1)
				{
					m_bmpHead.biBitCount = 1;		//B&W image
					m_bmpHead.biClrUsed = 2;
				}
				else if (bitspersample == 4)
				{
					m_bmpHead.biBitCount = 4;		//16 colors gray scale
					m_bmpHead.biClrUsed = 16;
				}
				else
				{
					m_bmpHead.biBitCount = 8;		//gray scale
					m_bmpHead.biClrUsed = 256;
				}
			}
			else if (bitspersample == 4)
			{
				m_bmpHead.biBitCount = 4;			// 16 colors
				m_bmpHead.biClrUsed = 16;
			}
			else
			{
				m_bmpHead.biBitCount = 8;			//256 colors
				m_bmpHead.biClrUsed = 256;
			}
		...
		create(width,height, m_bmpHead.biBitCount, XIMAGE_FORMAT_TIF);	//image creation

		if (nullptr == m_pBuffer)
		{
			//cx_throw("CxImageTIF can't create image");
			return false;
		}
		...
		{
			alphaCreate();	//add alpha support for 32bpp tiffs
		}
		if (samplesperpixel == 2 && bitspersample == 8)
		{
			alphaCreate();	//add alpha support for 8bpp + alpha
		}
#endif //CXIMAGE_SUPPORT_ALPHA

		TIFFGetField(m_tif, TIFFTAG_COMPRESSION, &compression);

		setCodecOption(compression, XIMAGE_FORMAT_TIF); // <DPR> save original compression type
		...
		else
		{
			...
			if (pal == NULL)
			{
				//cx_throw("Unable to allocate TIFF palette");
				return false;
			}
			...
			if (!BIG_palette)
			{ // + VK (BIG palette is stored until image is ready)
			...
			if (bitsize > (int32_t)(m_bmpHead.biSizeImage*samplesperpixel))
				bitsize = m_bmpHead.biSizeImage*samplesperpixel;

			if (bitsize < (int32_t)(m_extInfo.dwEffWidth*rowsperstrip))
				bitsize = m_extInfo.dwEffWidth*rowsperstrip;
			...
			if (bits16)
			{
				row_shifts = (uint8_t*)malloc(height);
			}
#endif
				...
				for (y = 0; y < nrow; y++) 
				{
					...
					if (samplesperpixel == 1) 
					{
						if (BIG_palette)
							...
							{
								int32_t offset16 = (nrow - y - 1)*line16;		// + VK
								MoveBitsPal(m_extInfo.pImage + m_extInfo.dwEffWidth * (height - ys - nrow + y),
								 bits16 + offset16, width, bitspersample, pal);
							}
							else
								MoveBitsPal(m_extInfo.pImage + m_extInfo.dwEffWidth * (height - ys - nrow + y),
									bits + offset, width, bitspersample, pal);
						else if ((bitspersample == m_bmpHead.biBitCount) ||
							(bitspersample == 16))	//simple 8bpp, 4bpp image or 16bpp
							memcpy(m_extInfo.pImage + m_extInfo.dwEffWidth*(height - ys - nrow + y), bits + offset, min((unsigned)line, m_extInfo.dwEffWidth));
						else
							MoveBits(m_extInfo.pImage + m_extInfo.dwEffWidth * (height - ys - nrow + y),
							bits + offset, width, bitspersample);
					}
					else if (samplesperpixel == 2) 
					{ //8bpp image with alpha layer
						int32_t xi = 0;
						int32_t ii = 0;
					...
					{ //photometric==PHOTOMETRIC_CIELAB
						...
						{ //fix image
							create(width, height, 24, XIMAGE_FORMAT_TIF);
#if CXIMAGE_SUPPORT_ALPHA
			...
			if (row_shifts && (samplesperpixel == 1) && (bitspersample == 16) && !BIG_palette) 
			{
				...
				for (y = 0; y < height; y++) 
				{
					if (min_row_shift < row_shifts[y]) 
					{
						...
						uint8_t* data = m_extInfo.pImage + m_extInfo.dwEffWidth * y;
...
////////////////////////////////////////////////////////////////////////////////
	...
	void xImageTIF::TileToStrip(uint8* out, uint8* in, uint32 rows, uint32 cols, int32_t outskew, int32_t inskew)
	{
		while (rows-- > 0) {
			uint32 j = cols;
			while (j-- > 0)
	...
	////////////////////////////////////////////////////////////////////////////////
	TIFF* xImageTIF::tiffOpenEx(xFile * hFile)
	{
	...
	////////////////////////////////////////////////////////////////////////////////
	void xImageTIF::tiffCloseEx(TIFF* tif)
	{
		if (tif) TIFFClose(tif);
	}
	////////////////////////////////////////////////////////////////////////////////
	void xImageTIF::MoveBits(uint8_t* dest, uint8_t* from, int32_t count, int32_t bpp)
	{
		int32_t offbits = 0;
		uint16 w;
		uint32 d;
	...
	////////////////////////////////////////////////////////////////////////////////
	void xImageTIF::MoveBitsPal(uint8_t* dest, uint8_t*from, int32_t count, int32_t bpp, RGBQUAD* pal)
	{
		int32_t offbits = 0;
		uint32 d;
		uint16 palidx;
